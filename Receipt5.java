 ...
 * Receipt5.java
 * V1.0.0 (Final)
 * Imagine being the final release version at 1.0.0
 * 
 * By Lemon
 *
 * Only added the discount thing.. cuz too lazy
 * You can add whatever, Just do
 */
...
public class Receipt5
{
    ...
    public static String orderNumberLen(int type,double num)
    {
        ...
        else if(type == 5) //Case of discount
        {
            if(Math.abs((int) num) >= 0 && Math.abs((int) num) <= 9)
            {
                text = "*    Discount        "+ String.format("%.2f" , num) +"%           *";
            } 
            else if(Math.abs((int) num) >= 10 && Math.abs((int) num) <= 99)
            {
                text = "*    Discount        "+ String.format("%.2f" , num) +"%          *";
    ...
    public static void main(String [] args)
    {
        ...
        System.out.println("How much discount? (in percent without the sign, e.g 50 for 50%)");
        ...
        */
        ...
        String DiscountLine = orderNumberLen(5, discount);
        String TotalLine = orderNumberLen(4, Finaltotal);
